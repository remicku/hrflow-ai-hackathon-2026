...
const SENIORITY_COLOR: Record<string, string> = {
  ...
  senior: 'text-violet-400 bg-violet-400/10 border-violet-400/30',
};
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white overflow-x-hidden">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 right-0 w-[500px] h-[500px] rounded-full bg-indigo-600/8 blur-3xl" />
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] rounded-full bg-violet-600/8 blur-3xl" />
      </div>
      ...
      <div className="relative max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="flex items-center gap-3 mb-12">
          ...
          <span className="text-slate-400 text-sm">Candidate Briefing</span>
        </div>
        ...
        <div className="grid lg:grid-cols-[1fr_380px] gap-8">
          ...
          <div className="space-y-6 animate-slide-up">
            ...
            <div className="glass-card rounded-2xl p-6">
              <div className="flex items-start gap-5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <h1 className="text-xl font-bold text-white">{brief.candidate_name}</h1>
                      <p className="text-slate-400 text-sm mt-0.5">{brief.current_title || 'No title provided'}</p>
                    </div>
                    <span
                      className={`inline-flex items-center text-xs font-semibold px-3 py-1.5 rounded-full border capitalize ${SENIORITY_COLOR[brief.seniority] ?? 'text-slate-400 bg-slate-700/50 border-slate-600'}`}
                    >
                      {brief.seniority}
                    </span>
                  </div>
              ...
              {/* Skills */}
              <div className="mt-6">
                <p className="text-slate-500 text-xs uppercase tracking-wider mb-3 font-semibold">Top Skills</p>
                <div className="flex flex-wrap gap-2">
                  {brief.top_skills.slice(0, 8).map((skill) => (
                    <span
                      key={skill}
                      className="px-3 py-1 bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 text-sm rounded-lg font-medium"
                    >
                      {skill}
                    </span>
                  ))}
            ...
            {profile.experiences.length > 0 && (
              <div className="glass-card rounded-2xl p-6">
                <p className="text-slate-500 text-xs uppercase tracking-wider mb-4 font-semibold flex items-center gap-2">
                  ...
                  {profile.experiences.slice(0, 3).map((exp, i) => (
                    <div key={i} className="flex gap-4">
                      ...
                      <div className="pb-4">
                        ...
                        <p className="text-slate-400 text-xs">{exp.company} · {Math.round(exp.duration_years * 10) / 10} yrs</p>
                        {exp.summary && (
                          <p className="text-slate-500 text-xs mt-1 line-clamp-2">{exp.summary}</p>
                        )}
                      </div>
                    </div>
                  ))}
          ...
          <div className="space-y-6 animate-slide-up" style={{ animationDelay: '0.1s' }}>
            ...
            <div className="glass-card rounded-2xl p-6">
              <div className="flex items-center gap-2 mb-5">
                ...
                <p className="text-white font-semibold">Interview Structure</p>
              </div>
              <div className="space-y-3">
                {INTERVIEW_STEPS.map((step, i) => (
                  <div key={step.category} className="flex items-start gap-3">
                    <div className="w-6 h-6 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-xs text-slate-400 font-bold shrink-0 mt-0.5">
                      {i + 1}
                    </div>
                    <div>
                      <p className="text-slate-200 text-sm font-medium">{CATEGORY_LABELS[step.category]}</p>
            ...
            <div className="bg-amber-500/5 border border-amber-500/20 rounded-xl p-5">
              <p className="text-amber-400 text-sm font-semibold mb-2">Before you start</p>
            ...
            <button
              onClick={onStart}
              className="w-full py-4 px-6 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-semibold text-base transition-all duration-200 shadow-xl shadow-indigo-500/25 flex items-center justify-center gap-3 group"
            >
              Begin Interview
